...
# -*- coding: utf-8 -*-
"""
الملف الموحد لاستخراج النصوص من الأرشيفات والمستندات والصور، مع دعم تقسيم الملفات الكبيرة.
يجمع هذا الملف وظائف:
- فك ضغط ZIP, RAR, TAR, GZ
- استخراج النصوص من PDF (مع OCR اختياري)، Excel، Word، HTML، قواعد البيانات
- تقسيم الملفات النصية الكبيرة إلى ملفات منفصلة
- معالجة المجلدات بالكامل

الاستخدام:
  استخراج: python script.py [--via-excel] [--ocr] ملف/مجلد ...
  تقسيم:   python script.py --split ملف_نصي_كبير ...
"""

import sys
import site

# إضافة مسار حزم المستخدم (لحل مشكلة عدم رؤية المكتبات عند التشغيل من القائمة اليمنى)
try:
...
# إخماد تحذيرات المكتبات (اختياري)
warnings.filterwarnings("ignore", category=UserWarning)

# ============ استيراد المكتبات الاختيارية مع محاولة التثبيت التلقائي ============
def check_and_import(module_name, package_name=None):
    """فحص وجود مكتبة وإرجاع الوحدة إن وجدت، وإلا None"""
    try:
...
if pd:
    try:
        from openpyxl import load_workbook  # جزء من pandas/openpyxl
    except ImportError:
        pd = None
...
GZ_EXTENSIONS = {'.gz'}  # للملفات المضغوطة المفردة
...
def get_unique_dirname(target_dir, base_name, suffix="_extracted"):
    """إنشاء اسم مجلد فريد بجانب العنصر"""
    dir_path = os.path.join(target_dir, base_name + suffix)
    if not os.path.exists(dir_path):
        return dir_path
    counter = 1
    while True:
        new_path = os.path.join(target_dir, f"{base_name}{suffix}_{counter}")
...
def convert_timestamp(value):
    """تحويل الطوابع الزمنية إلى تنسيق عربي مقروء (ص/م)"""
    if pd is None:
        return str(value)
    try:
...
def extract_html_to_text(html_path, output_dir):
    if bs4 is None:
        # معالجة بسيطة بدون BeautifulSoup
        try:
...
def extract_pdf_advanced(pdf_path, output_dir, use_ocr=True):
    """
    استخراج متقدم من PDF:
    - نص كل صفحة
    - جداول
    - صور مع حفظها ومحاولة OCR
    - بيانات وصفية
    - استخدام أدوات خارجية: pdfimages (من poppler) و exiftool إن وجدت
    """
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # محاولة OCR للصفحات التي لا تحتوي على نص (إذا طلب المستخدم)
            ...
            # استخدام أدوات خارجية إن وجدت
            # pdfimages لاستخراج الصور الفعلية
            if shutil.which("pdfimages"):
                print("   🖼️ استخراج الصور الفعلية باستخدام pdfimages...")
                try:
                    subprocess.run(["pdfimages", "-all", pdf_path, os.path.join(images_dir, "embedded")], 
                                   check=True, capture_output=True)
                except subprocess.CalledProcessError as e:
                    print(f"      ⚠️ فشل pdfimages: {e.stderr.decode() if e.stderr else ''}")
            else:
                print("   ⚠️ pdfimages غير مثبت. لتثبيته: sudo apt install poppler-utils")

            # exiftool لاستخراج بيانات الصور
            if shutil.which("exiftool"):
                print("   🏷️ استخراج بيانات الصور باستخدام exiftool...")
                try:
                    with open(os.path.join(images_dir, "image_metadata.csv"), 'w') as f:
                        subprocess.run(["exiftool", "-csv", images_dir], stdout=f, check=True)
                except subprocess.CalledProcessError:
                    print(f"      ⚠️ فشل exiftool")
            else:
                print("   ⚠️ exiftool غير مثبت. لتثبيته: sudo apt install exiftool")
...
def extract_archive_to_files(archive_path, output_dir, archive_type="zip"):
    """فك ضغط الأرشيف واستخراج كل ملف نصي إلى ملف في output_dir مع الحفاظ على الهيكل"""
...
def extract_tar_to_files(tar_path, output_dir):
    """استخراج أرشيف tar (بجميع صيغ الضغط) إلى ملفات منفصلة"""
...
def extract_gz_to_file(gz_path, output_dir):
    """فك ضغط ملف .gz مفرد (ليس tar) إلى ملف نصي"""
    try:
...
def extract_single_file_to_text(file_path, output_dir):
    """نسخ ملف نصي عادي إلى مجلد الإخراج مع إضافة امتداد .txt إذا لزم الأمر"""
...
# ============ المعالج الرئيسي للاستخراج ============
def process_single_item(item_path, via_excel=False, use_ocr=False):
    """معالجة عنصر واحد (ملف أو مجلد) وإنشاء مجلد مخصص له"""
    ...
    # إنشاء مجلد الإخراج بجانب العنصر
    ...
    if os.path.isfile(item_path):
        ...
        # تجميع قائمة الملفات في المجلد ومعالجتها
        all_files = []
        processed = 0
        skipped = 0
...
def split_big_text_file(file_path):
    """تقسيم ملف نصي كبير (ناتج من الإصدار القديم) إلى ملفات منفصلة"""
    if not os.path.isfile(file_path):
    ...
    target_dir = get_unique_dirname(output_dir, base_name, suffix="_split")
...
# ============ الوظيفة الرئيسية ============
def main():
    parser = argparse.ArgumentParser(description="أداة متكاملة لاستخراج النصوص من الأرشيفات والمستندات وتقسيم الملفات الكبيرة.")
    parser.add_argument('--split', action='store_true', help='تقسيم ملف نصي كبير إلى ملفات منفصلة')
    parser.add_argument('--via-excel', action='store_true', help='استخدام Excel كوسيط لاستخراج قواعد البيانات')
    parser.add_argument('--ocr', action='store_true', help='تشغيل OCR على صفحات PDF التي لا تحتوي على نص')
    parser.add_argument('files', nargs='+', help='الملفات أو المجلدات المراد معالجتها')
    ...
    if args.split:
        # وضع التقسيم
        print("✂️ وضع تقسيم الملفات الكبيرة")
        ...
        print(f"\n🎯 تم العثور على {len(args.files)} عنصر للمعالجة:")
        for i, item in enumerate(args.files, 1):
            print(f"\n[{i}/{len(args.files)}] {'='*50}")
            results = process_single_item(item, via_excel=args.via_excel, use_ocr=args.ocr)
