...
# -*- coding: utf-8 -*-
"""
Ultimate Text Classifier v3 - Fixed Version
Advanced text classification system with semantic deduplication and active learning
...
try:
    ...
    print(f"Missing required packages. Please install: pip install requests beautifulsoup4 faiss-cpu sentence-transformers")
    sys.exit(1)
...
class UltimateTextClassifier:
    """
    Advanced text classifier with semantic deduplication and active learning
    """
    
    def __init__(self, 
                 model_name: str = "intfloat/multilingual-e5-large",
                 ollama_model: str = "qwen2.5",
                 output_dir: str = "~/classified_ultimate"):
        
        self.model_name = model_name
        self.ollama_model = ollama_model
        self.output_dir = Path(output_dir).expanduser()
        self.knowledge_file = Path("user_knowledge.json")
        self.embeddings_dim = 1024  # For multilingual-e5-large
    ...
    def extract_embeddings(self, texts: List[str]) -> np.ndarray:
        """Extract embeddings for texts using sentence transformer"""
        if not self.sentence_model:
            logger.info(f"Loading sentence transformer model: {self.model_name}")
            self.sentence_model = SentenceTransformer(self.model_name)
            
        # Prepare texts for embedding (add instruction prefix for E5 models)
        prepared_texts = [f"query: {text}" for text in texts]
        embeddings = self.sentence_model.encode(prepared_texts)
    ...
    def find_duplicates(self, threshold: float = 0.95) -> List[Tuple[int, int, float]]:
        """Find semantically similar texts using FAISS"""
        ...
        distances, indices = self.vector_index.search(
            ...
            k=min(10, self.vector_index.ntotal)
        )
        ...
        for i in range(len(distances)):
            for j in range(len(distances[i])):
                if i != indices[i][j] and distances[i][j] > threshold:
                    pair = tuple(sorted([i, indices[i][j]]))
                    if pair not in seen_pairs:
                        duplicates.append((i, indices[i][j], distances[i][j]))
                        seen_pairs.add(pair)
...
"""
    ...
    def process_directory(self, directory_path: Path) -> List[Dict]:
        ...
        self.perform_deduplication(results)
    ...
    def perform_deduplication(self, results: List[Dict]):
        """Remove semantically similar content using FAISS"""
        if not results:
            return
            
        # Extract text content for deduplication
        texts = []
        for result in results:
            source_file = Path(result["source_file"])
            if source_file.suffix.lower() in ['.txt', '.py', '.js', '.html', '.css', '.json', '.xml', '.csv', '.md', '.yml', '.yaml']:
                try:
        ...
        if len(texts) < 2:
            logger.info("Not enough content for deduplication")
            return
        ...
        # Find duplicates
        duplicates = self.find_duplicates(threshold=0.95)
        
        logger.info(f"Found {len(duplicates)} semantically similar pairs")
        
        # Log duplicate information
        for idx1, idx2, similarity in duplicates:
            logger.info(f"Similar texts (similarity: {similarity:.3f}): {idx1} <-> {idx2}")
...
"""
        ...
        # Save report
        report_file = self.output_dir / "reports" / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    parser = argparse.ArgumentParser(description="Ultimate Text Classifier v3 - Fixed Version")
    parser.add_argument("--input", "-i", type=str, help="Input file or directory path")
    parser.add_argument("--output", "-o", type=str, default="~/classified_ultimate", help="Output directory")
    ...
    classifier = UltimateTextClassifier(
        ollama_model=args.model,
        output_dir=args.output
    )
    
    if not args.input:
        print("Ultimate Text Classifier v3 - Fixed Version")
        print("="*50)
        print("Options:")
